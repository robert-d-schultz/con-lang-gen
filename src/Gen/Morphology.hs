...
makeLexicalInflection :: [ConsCluster] -> [Phoneme] -> [ConsCluster] -> [Tone] -> InflectionMap -> (LexCat, Int, Int, Int, Int) -> RVar [Morpheme]
makeLexicalInflection onsets nucs codas tones inflMap (lc, i, j, k, l) = do
  ...
  trans <- makeExponentSystems lc Transfix l onsets nucs codas tones inflMap
  return (part ++ pref ++ suff ++ trans)

makeExponentSystems :: LexCat -> MorphType -> Int -> [ConsCluster] -> [Phoneme] -> [ConsCluster] -> [Tone] -> InflectionMap -> RVar [Morpheme]
makeExponentSystems _ _ 0 _ _ _ _ _ = return []
makeExponentSystems lc morphType i onsets nucs codas tones gramSys = (++) <$> makeExponentSystems lc morphType (i-1) onsets nucs codas tones gramSys <*> makeExponentSystem lc morphType i onsets nucs codas tones gramSys

makeExponentSystem :: LexCat -> MorphType -> Int -> [ConsCluster] -> [Phoneme] -> [ConsCluster] -> [Tone] -> InflectionMap -> RVar [Morpheme]
makeExponentSystem lc Transfix i onsets nucs codas tones gramSys = do
  let (gen,ani,cas,num,def,spe,top,per,hon,pol,ten,asp,moo,voi,evi,tra,vol) = cleanInflectionSys gramSys lc Transfix i
  let combos = (,,,,,,,,,,,,,,,,) <$> gen <*> ani <*> cas <*> num <*> def <*> spe <*> top <*> per <*> hon <*> pol <*> ten <*> asp <*> moo <*> voi <*> evi <*> tra <*> vol
  roots <- replicateM (length combos) (makeTransfix onsets nucs codas 3) -- 3 should be a parameter
  let morphs = zipWith (\x y -> SemiticRoot x Transfix y) (InflMeaning lc <$> combos) roots
  return morphs
makeExponentSystem lc morphType i onsets nucs codas tones gramSys = do
  let (gen,ani,cas,num,def,spe,top,per,hon,pol,ten,asp,moo,voi,evi,tra,vol) = cleanInflectionSys gramSys lc morphType i
  let combos = (,,,,,,,,,,,,,,,,) <$> gen <*> ani <*> cas <*> num <*> def <*> spe <*> top <*> per <*> hon <*> pol <*> ten <*> asp <*> moo <*> voi <*> evi <*> tra <*> vol
...
cleanInflectionSys :: InflectionMap -> LexCat -> MorphType -> Int -> ([Express Gender], [Express Animacy], [Express Case], [Express Number], [Express Definiteness], [Express Specificity], [Express Topic], [Express Person], [Express Honorific], [Express Polarity], [Express Tense], [Express Aspect], [Express Mood], [Express Voice], [Express Evidentiality], [Express Transitivity], [Express Volition])
