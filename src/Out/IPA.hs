...
searchCIPA :: Place -> Manner -> Phonation -> Maybe Text
searchCIPA p m h = snd <$> filt where
  conso = filter (not . null . snd) $ concat c
  filt = find (\(Consonant pc mc hc, _) -> p == pc && m == mc && h == hc) conso
...
searchVIPA :: Height -> Backness -> Roundedness -> Length -> Tone -> Maybe Text
searchVIPA h b r l t = snd <$> filt where
  vows = filter (not . null . snd) $ concat v
  filt = find (\(Vowel hv bv rv lv tv, _) -> h == hv && b == bv && r == rv && l == lv && t == tv) vows
